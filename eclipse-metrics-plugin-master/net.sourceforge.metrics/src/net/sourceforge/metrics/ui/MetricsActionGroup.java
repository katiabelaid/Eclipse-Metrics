...
/**
 * view actions for the MetricsView (Recalculate and Export XML - CSV and analysis file)
 * 
 * @author Frank Sauer
 */
public class MetricsActionGroup extends ActionGroup {
	...
	private ExportAnalysisFileAction exportAnalysisFileAction;
	...
	private void createActions() {
		...
		exportAnalysisFileAction = new ExportAnalysisFileAction();
	...
	void fillToolBar(IToolBarManager toolBar) {
		...
		toolBar.add(exportCSVAction);
		toolBar.add(exportAnalysisFileAction);
	...
	void fillViewMenu(IMenuManager menu) {
		...
		menu.add(exportAnalysisFileAction);
	...
	private class ExportCSVAction extends Action {
		public ExportCSVAction() {
			super("&Export CSV...", MetricsPlugin.createImage("export_csv.gif"));
			setToolTipText("&Export CSV...");
		}
		...
		public void run() {
			try {
				...
				Log.logError("ExportCSVAction::run", e);
			}
		}
	}
	
	private class ExportAnalysisFileAction extends Action {
		public ExportAnalysisFileAction() {
			super("&Export analysis file...", MetricsPlugin.createImage("export_analysis_file.gif"));
			setToolTipText("&Export analysis file...");
		}
		/**
		 * @see org.eclipse.jface.action.Action#run()
		 */
		...
		public void run() {
			try {
				getView().exportAnalysisFile();
			} catch (RuntimeException e) {
				Log.logError("ExportAnalysisFileAction::run", e);
			}
		}
	}
	...
	public void enable() {
		...
		exportAnalysisFileAction.setEnabled(true);
	...
	public void disable() {
		...
		exportAnalysisFileAction.setEnabled(false);
		graphAction.setEnabled(false);
	}
