...
import net.sourceforge.metrics.internal.analysisFile.AnalysisFileExport;
import net.sourceforge.metrics.internal.csv.CSVexport;
...
public class MetricsView extends ViewPart implements ISelectionListener, IMetricsProgressListener, IPropertyChangeListener {
		...
		if (selection != null) {
			...
			if (fileName != null) {
				...
				try {
					...
					AbstractMetricSource ms = Dispatcher.getAbstractMetricSource(selection);
					CSVexport.doExport(ms,CSVfileWriter);
				} catch (IOException e) {
					e.printStackTrace();
				}	
			}
	...
	public void exportAnalysisFile() {
		...
		if (CSVfile != null) {
			...
			if (fileName != null) {
				...
				try {
					...
					AnalysisFileExport.doExport(outputFile,CSVfile);
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
